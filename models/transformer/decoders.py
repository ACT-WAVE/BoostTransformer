...
from models.transformer.attention import MultiHeadAttention, BoostCrossAttention
...
class MeshedDecoderLayer(Module):
    def __init__(self, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1, self_att_module=None,
        ...
        self.enc_att1 = BoostCrossAttention(d_model, d_model//4, d_v//2, h, dropout)
        
        self.enc_att2 = BoostCrossAttention(d_model, d_model//2, d_v, h, dropout)
        
        self.enc_att3 = MultiHeadAttention(d_model, d_k, d_v, h, dropout, can_be_stateful=False,
    ...
    def forward(self, input, x1,x2,x3, att1,att2, mask_pad, mask_self_att):  # decoder(seq, enc_output, mask_enc)
        self_att = self.self_att(input, input, input, mask_self_att)
        self_att = self_att * mask_pad

        enc_cross3, att3 = self.enc_att3(self_att, x3, x3, return_att=True)
        enc_cross3 = enc_cross3 * mask_pad
        enc_cross1 = self.enc_att1(self_att, att1, att3, x1) * mask_pad
        enc_cross2 = self.enc_att2(self_att, att2, att3, x2) * mask_pad
        
        alpha1 = torch.sigmoid(self.fc_alpha1(torch.cat([self_att, enc_cross1], -1)))  # 10 15 512
        alpha2 = torch.sigmoid(self.fc_alpha2(torch.cat([self_att, enc_cross2], -1)))  # 10 15 512
        alpha3 = torch.sigmoid(self.fc_alpha3(torch.cat([self_att, enc_cross3], -1)))  # 10 15 512
        enc_att = (enc_cross1 * alpha1 + enc_cross2 * alpha2 + enc_cross3 * alpha3) / np.sqrt(3)
...
class BoostMeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
        ...
        super(BoostMeshedDecoder, self).__init__()
    ...
    def forward(self, input, x1,x2,x3, att1,att2):
        ...
        for l in self.layers:
            out = l(out,  x1,x2,x3, att1,att2,  mask_queries, mask_self_attention)
