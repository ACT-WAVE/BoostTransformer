...
class ImageField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, 
                 origin_img_path=None, image_size=256, train=False):
        self.origin_img_path = origin_img_path

        PreTrans_train=transforms.Compose([
            transforms.RandomRotation(degrees=5, expand=True),
            transforms.RandomResizedCrop(size=(image_size, image_size), scale=(0.8, 1.2), ratio=(0.9, 1.1)),
            transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.01),
            transforms.GaussianBlur(kernel_size=(3,3), sigma=(0.1, 1)),
            transforms.ToTensor(),
            transforms.RandomErasing(scale=(0.01,0.1), ratio=(0.25,4)),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            ])
        PreTrans_test = transforms.Compose([
            transforms.Resize(size=(image_size, image_size)),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            ])
        
        self.PreTrans = PreTrans_train if train else PreTrans_test
        super(ImageField, self).__init__(preprocessing, postprocessing)


    def preprocess(self, x, avoid_precomp=False):
        image_name = x.split('/')[-1]
        I = Image.open(os.path.join(self.origin_img_path, image_name)).convert('RGB')
        I = self.PreTrans(I)
        return I
